...
public class AssistResidentController {
    ...
    public ResponseResult exportExcelByCondition(@PathVariable(value = "condition") String condition,HttpServletResponse response) throws Exception {
        ...
        String error_description = "";
        HashMap<String,String> map = CommonUtil.getCondtionMap(condition);
        List<AssistResidentVo> assistResidentVos=assistResidentService.getAssistResidentVosByCondition(map);
        //有记录
        if(assistResidentVos.size()>0) {
            HSSFWorkbook workbook= assistResidentService.exportExcel(assistResidentVos);
            try{
                response = CommonUtil.setExcelResponse(response,"帮扶人员信息管理");
                // 将文件输出到客户端浏览器
